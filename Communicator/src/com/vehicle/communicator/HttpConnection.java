...
public class HttpConnection implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            e.printStackTrace();
            handler.sendMessage(Message.obtain(handler, DID_ERROR, e.getMessage()));
        }
        ConnectionManager.getInstance().didComplete(this);
    }

    private void processEntity(HttpEntity entity, int statusCode) throws IllegalStateException,
            IOException {
...
//        }
    ...
    private boolean processForError(String result) {
        ...
        if (result.contains("status_code")) {
            if (!result.contains("\"status_code\":201") || !result.contains("\"status_code\":200")) {
                isError = true;
            }
