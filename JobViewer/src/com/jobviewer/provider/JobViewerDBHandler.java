...
public class JobViewerDBHandler {
	...
	public static void saveImage(Context context, ImageObject image) {
		...
		values.put(JobViewerProviderContract.Image.IMAGE_EXIF,
				image.getImage_exif());
		values.put(JobViewerProviderContract.Image.EMAIL, image.getEmail());
		values.put(JobViewerProviderContract.Image.REFERENCE_ID,
				image.getReference_id());
		values.put(JobViewerProviderContract.Image.STAGE, image.getStage());
		context.getContentResolver().insert(
	...
	public static ImageObject getImageById(Context context, String id) {
		...
		if (cursor != null && cursor.moveToFirst()) {
			...
			imageObject.setEmail(cursor.getString(cursor
					.getColumnIndex(JobViewerProviderContract.Image.EMAIL)));

			imageObject
					.setReference_id(cursor.getString(cursor
							.getColumnIndex(JobViewerProviderContract.Image.REFERENCE_ID)));

			imageObject.setStage(cursor.getString(cursor
					.getColumnIndex(JobViewerProviderContract.Image.STAGE)));
		}
		cursor.close();
		return imageObject;
	}
	...
	public static List<ImageObject> getAllSavedImages(Context context) {
		...
		if (cursor != null && cursor.moveToFirst()) {
			do {
				...
				imageObject
						.setEmail(cursor.getString(cursor
								.getColumnIndex(JobViewerProviderContract.Image.EMAIL)));
				imageObject
						.setReference_id(cursor.getString(cursor
								.getColumnIndex(JobViewerProviderContract.Image.REFERENCE_ID)));
				imageObject
						.setStage(cursor.getString(cursor
								.getColumnIndex(JobViewerProviderContract.Image.STAGE)));
	...
	public static void saveBackLog(Context context, BackLogRequest request) {
		...
		if (!Utils.isNullOrEmpty(str)) {
			try {
				...
				if (Utils.isNullOrEmpty(new JobViewerSharedPref()
						.getSharedPref(context).getString(
	...
	public static void saveAddPhotoImage(Context context, ImageObject image) {
		try {
			...
			values.put(
					...
					image.getImage_exif());
			values.put(
					JobViewerProviderContract.AddPhotosScreenSavedImages.EMAIL,
					image.getEmail());
			values.put(
					JobViewerProviderContract.AddPhotosScreenSavedImages.REFERENCE_ID,
					image.getReference_id());
			values.put(
					JobViewerProviderContract.AddPhotosScreenSavedImages.STAGE,
					"");
			context.getContentResolver()
	...
	public static List<ImageObject> getAllAddCardSavedImages(Context context) {
		...
		if (cursor != null && cursor.moveToFirst() && cursor.getCount() > 0) {
			do {
				...
				imageObject
						.setEmail(cursor.getString(cursor
								.getColumnIndex(JobViewerProviderContract.AddPhotosScreenSavedImages.EMAIL)));
				imageObject
						.setStage(cursor.getString(cursor
								.getColumnIndex(JobViewerProviderContract.AddPhotosScreenSavedImages.STAGE)));
				imageObject
						.setReference_id(cursor.getString(cursor
								.getColumnIndex(JobViewerProviderContract.AddPhotosScreenSavedImages.REFERENCE_ID)));
				imageObject
						.setImage_exif(cursor.getString(cursor
	...
	public static void saveBreakShiftTravelCall(Context context,
		...
		values.put(
				JobViewerProviderContract.BreakTravelShiftCallTable.NO_OF_WORKS_COMPLETED,
				breakShiftTravelCall.getNoOfWorksCompleted());
