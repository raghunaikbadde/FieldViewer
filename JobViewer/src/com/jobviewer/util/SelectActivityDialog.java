...
public class SelectActivityDialog extends Activity implements
		...
		findViewById(R.id.dialog_ok).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (selected == -1)
					...
					Intent waterAbstractionActivity = new Intent(mContext,
							WaterAbstractionActivity.class);
					startActivity(waterAbstractionActivity);
				} else if (selected == 1) {
					jobViewerSharedPref.saveFromWork(mContext, false);
					jobViewerSharedPref.fromNoPhotsWork(mContext, false);
	...
	public void onConfirmStartWithNoPhotos() {
		...
		if (selected == -1)
			...
			jobViewerSharedPref.fromNoPhotsWork(mContext, true);
	...
	public void updateOkButtonDrawable(boolean isEnable) {
		if (isEnable) {
			dialog_ok.setBackground(mContext.getResources().getDrawable(
					R.drawable.dialog_red_button));
			dialog_ok.setEnabled(true);
		} else {
			dialog_ok.setBackground(mContext.getResources().getDrawable(
					R.drawable.dialog_dark_grey_button));
	...
	private boolean shouldNewWorkTravelArrivedSite(Context mContext) {
		...
		try {
			...
			if (jsonObject.has(Constants.FLAG_NEW_WORK_TRAVEL_ARRIVED_SITE)) {
				return jsonObject
						.getBoolean(Constants.FLAG_NEW_WORK_TRAVEL_ARRIVED_SITE);
			}
