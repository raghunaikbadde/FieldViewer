...
import com.jobviewer.map.JobViewerMapActivity;
...
public class PollutionActivity extends BaseActivity implements
	...
	private void initUI() {
		...
		mUpStreamEditText.addTextChangedListener(new GenericTextWatcher(
				mUpStreamEditText));
		mDownStreamEdiText.addTextChangedListener(new GenericTextWatcher(
				mDownStreamEdiText));
		...
		map_layout.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Intent mapIntent = new Intent(PollutionActivity.this,
						JobViewerMapActivity.class);
	...
	public static void setMapAnnotationImage(String imageString) {
		ImageObject imageObject = new ImageObject();
		imageObject.setCategory("pollution");
		imageObject.setImage_string(Constants.IMAGE_STRING_INITIAL
				+ imageString);
		mapImageId = Utils.generateUniqueID(context);
		...
		if (Utils.isInternetAvailable(context)) {
			...
			JobViewerDBHandler.saveImage(context, imageObject);
		}
	}

	private static Handler getMapImageHandler(ImageObject imageObject) {
	...
	public void onClick(View v) {
		...
		case R.id.nextButton:
			if (!getResources().getString(R.string.select_spinner_str)
					.equalsIgnoreCase(landPollutantsText.getText().toString())) {
				stringOfLandPollutants = landPollutantsText.getText()
						.toString();
			} else {
			...
			if (!getResources().getString(R.string.select_spinner_str)
					.equalsIgnoreCase(
							waterPollutantsTextView.getText().toString())) {
				stringOfWaterPollutants = waterPollutantsTextView.getText()
						.toString();
			} else {
			...
			if (ptwCheckbox.isChecked()) {
				if (upStreamImageObject != null) {
					JobViewerDBHandler.saveImage(getApplicationContext(),
							upStreamImageObject);
				}
				if (downSteamIamgeObject != null) {
					JobViewerDBHandler.saveImage(getApplicationContext(),
			...
			if (Utils.isInternetAvailable(PollutionActivity.this)) {
				...
				if (ptwCheckbox.isChecked()) {
					...
					if (downSteamIamgeObject != null) {
						sendUpStreamWorkImageToServer(downSteamIamgeObject);
					}
				...
				addPhotosActivityIntent.putExtra(
						Constants.KEY_IS_FROM_POLLUTION, true);
		...
		case R.id.spinnerLayoutadditionalED:
			...
			for (String mainValue : additionalEPDArray) {
				MultiChoiceItem item = new MultiChoiceItem();
				item.setText(mainValue);
				if (isSelctedBefore) {
	...
	private void sendPollutionReportToServer() {
		...
		if (ptwCheckbox.isChecked()) {
			...
			pollutionReportRequest.setWater_pollutants(stringOfWaterPollutants);
			data.put("water_pollutants", stringOfWaterPollutants);
			...
			if (upStreamImageObject != null
					&& !Utils.isNullOrEmpty(upStreamImageObject
							.getImage_string())) {
			...
			if (downSteamIamgeObject != null
					&& !Utils.isNullOrEmpty(downSteamIamgeObject
							.getImage_string())) {
				pollutionReportRequest
		...
		data.put("mapImageId", mapImageId);
		pollutionReportRequest.setMapImageId(mapImageId);
		Log.d(Utils.LOG_TAG,
				" url - : "
						+ CommsConstant.HOST
						+ CommsConstant.POLLUTION_REPORT_UPLOAD
						+ "/"
						+ mSharedPref.getSharedPref(context).getString(
		...
		try {
			...
			if (checkOutObject != null
				...
				mSharedPref.saveWorkId(context, checkOutObject.getWorkId());
		...
		if (Utils.isInternetAvailable(PollutionActivity.this)
			...
			Utils.SendHTTPRequest(
					...
					CommsConstant.HOST
							...
							+ "/"
							+ mSharedPref.getSharedPref(context).getString(
									JobViewerSharedPref.KEY_WORK_ID, ""), data,
					getPollutionReportHandler());
		} else {
	...
	public void validateUserInputs() {
		...
		if (ptlCheckbox.isChecked() && ptwCheckbox.isChecked()) {
			...
			if (ptwCheckbox.isChecked()) {
				...
				if (!mUpStreamEditText.getText().toString().trim().equals("")) {
					if (upStreamImageObject != null) {
						enableNextButton(true);
					} else {
						enableNextButton(false);
				...
				if (!mDownStreamEdiText.getText().toString().trim().equals("")) {
					if (downSteamIamgeObject != null) {
						enableNextButton(true);
					} else {
						enableNextButton(false);
	...
	private class GenericTextWatcher implements TextWatcher {
		...
		public void afterTextChanged(Editable editable) {
			...
			switch (view.getId()) {
			case R.id.upstream_edittext:
				validateUserInputs();

				break;
			case R.id.downstream_edittext:
				validateUserInputs();
