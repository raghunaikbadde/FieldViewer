...
public class NewWorkActivity extends BaseActivity implements OnClickListener,
	...
	private RelativeLayout pollutionCheckParent;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent != null && intent.hasExtra(Constants.KEY_ABSTRACTION_VALUE)) {
			abstractionValue = intent
					.getStringExtra(Constants.KEY_ABSTRACTION_VALUE);
		}
		mJVSharedPref = new JobViewerSharedPref();
		if (abstractionValue.equals("")) {
			abstractionValue = mJVSharedPref.getSharedPref(context).getString(
					JobViewerSharedPref.WATER_ABSTRACTION_VALUE, "");
		}
		initUI();
	}

	private void initUI() {
		pollutionCheckParent = (RelativeLayout) findViewById(R.id.pollutionCheckParent);
		...
		if (mJVSharedPref.getSharedPref(context).getBoolean(
				JobViewerSharedPref.KEY_WORK_NO_PHOTOS_DATA, false)) {
			pollutionCheckParent.setVisibility(View.GONE);
		} else {
			pollutionCheckParent.setVisibility(View.VISIBLE);
		}
	}
			...
			if (isValidUserInput) {
				if (mPollutionCheckBox.isChecked()) {
					ConfirmDialog confirmDialog = new ConfirmDialog(context,
							this, Constants.POLLUTION_CONFIRMATION);
					confirmDialog.show();
				} else {
	...
	private void saveWorkStartInBackLogDB() {
		...
		Utils.saveTimeSheetInBackLogTable(this,
				Utils.workStartTimeSheetRequest, api,
	...
	private void executeWorkCreateService() {
		...
		if (mJVSharedPref.getSharedPref(context).getBoolean(
				JobViewerSharedPref.KEY_FROM_WORK, false)) {
			...
			public void handleMessage(Message msg) {
				...
				case HttpConnection.DID_SUCCEED:
					...
					JobViewerDBHandler.saveCheckOutRemember(context,
							checkOutRemember);
					new JobViewerSharedPref().saveWorkId(context,
							decodeFromJsonString.getId());
	...
	private void startEndActvity() {
		if (CheckAndCcontinueNoWork()) {
			...
			String jsonStr = JobViewerDBHandler
					.getJSONFlagObject(NewWorkActivity.this);
			try {

				JSONObject jsonObject = new JSONObject(jsonStr);
				jsonObject.put(Constants.WorkWithNoPhotosStartedAt,
						currentTimeStamp);
				JobViewerDBHandler.saveFlaginJSONObject(NewWorkActivity.this,
						jsonObject.toString());
	...
	private void insertWorkStartTimeIntoHoursCalculator() {
		BreakShiftTravelCall breakShiftTravelCall = JobViewerDBHandler
				.getBreakShiftTravelCall(NewWorkActivity.this);
		if (breakShiftTravelCall == null) {
			breakShiftTravelCall = new BreakShiftTravelCall();
		}
		breakShiftTravelCall.setWorkStartTime(String.valueOf(System
				.currentTimeMillis()));
		JobViewerDBHandler.saveBreakShiftTravelCall(NewWorkActivity.this,
	...
	private void sendWorkStartTimeRequest() {
		...
		data.put("override_timestamp",
				workStartTimeRequest.getOverride_timestamp());
	...
	private void setNewWorkTravelArrivedSite(Context mContext) {
		...
		try {
			...
			if (jsonObject.has(Constants.FLAG_NEW_WORK_TRAVEL_ARRIVED_SITE)) {
				jsonObject.remove(Constants.FLAG_NEW_WORK_TRAVEL_ARRIVED_SITE);
			}

			jsonObject.put(Constants.FLAG_NEW_WORK_TRAVEL_ARRIVED_SITE, true);
