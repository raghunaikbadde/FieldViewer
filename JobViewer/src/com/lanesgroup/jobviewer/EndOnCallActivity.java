...
public class EndOnCallActivity extends BaseActivity implements OnClickListener {
	...
	private void initUI() {
		...
		shift_end_time_text = (TextView) findViewById(R.id.shift_end_time_text);
	...
	private void updateDetailsOnUI() {
		...
		checkOutRemember = JobViewerDBHandler
				.getCheckOutRemember(EndOnCallActivity.this);
		String vehicleRegistrationNumber = checkOutRemember
		...
		if (checkOutRemember.getJobSelected().contains("shift")) {
			...
			mEndOnCall.setText(getResources().getString(
					R.string.end_shift_screen_title));
			shift_end_time_text.setText(getResources().getString(
					R.string.shift_end_time));
			user_text.setText(getResources().getString(
					R.string.user_ending_shift));
		} 
		...
		if (bundle != null && bundle.containsKey(Utils.CALLING_ACTIVITY)) {
			if (bundle.getString(Utils.CALLING_ACTIVITY).contains(
					"EndShiftReturnVehicleActivity")) {
	...
	public void onClick(View v) {
		if (v == mCancel) {
			...
			Intent intent = new Intent(EndOnCallActivity.this,
					ChangeTimeDialog.class);
			if (checkOutRemember.getJobSelected().contains("shift")) {
				...
				Utils.endShiftRequest.setReference_id(checkOutRemember
						.getVistecId());
				...
				Utils.callEndTimeRequest.setReference_id(checkOutRemember
						.getVistecId());
	...
	private void saveEndCallOrEndShiftInBackLogDB() {
		if (checkOutRemember.getJobSelected().contains("shift")) {
			...
			Utils.saveTimeSheetInBackLogTable(this, Utils.endShiftRequest,
					CommsConstant.END_SHIFT_API, "TimeSheetServiceRequests");
		} else {
			...
			Utils.saveTimeSheetInBackLogTable(this, Utils.callEndTimeRequest,
					CommsConstant.END_ON_CALL_API, "TimeSheetServiceRequests");
		}
	}
	...
	private void executeEndOnCallService() {
		...
		if (!Utils.isNullOrEmpty(Utils.callEndTimeRequest.getReference_id())) {
	...
	private void executeShiftEndService() {
		...
		if (!Utils.isNullOrEmpty(Utils.endShiftRequest.getReference_id())) {
	...
	private void insertShiftEndTimeIntoHoursCalculator() {
		BreakShiftTravelCall breakShiftTravelCall = JobViewerDBHandler
				.getBreakShiftTravelCall(EndOnCallActivity.this);

		String timeToStore = String.valueOf(System.currentTimeMillis());
		if (Utils.endShiftRequest.getIs_overriden().equalsIgnoreCase(
				ActivityConstants.TRUE)) {
			Log.d(Utils.LOG_TAG, "shift end time overridden");
			timeToStore = Utils
					.getMillisFromFormattedDate(Utils.endShiftRequest
							.getOverride_timestamp());
		}
		...
		JobViewerDBHandler.saveBreakShiftTravelCall(EndOnCallActivity.this,
	...
	private void insertCallEndTimeIntoHoursCalculator() {
		BreakShiftTravelCall breakShiftTravelCall = JobViewerDBHandler
				.getBreakShiftTravelCall(EndOnCallActivity.this);
		String timeToStore = String.valueOf(System.currentTimeMillis());
		if (Utils.callEndTimeRequest.getIs_overriden().equalsIgnoreCase(
				ActivityConstants.TRUE)) {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == Constants.RESULT_CODE_CHANGE_TIME
			...
			if (checkOutRemember.getJobSelected().contains("shift")) {
				...
				mEndTime.setTextColor(this.getResources()
