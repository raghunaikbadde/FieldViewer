...
public class ActivityPageActivity extends BaseActivity implements
	...
	private void updateDetailsOnUI() {
		...
		if (breakShiftTravelCall != null) {
			numberOfBreaks = String.valueOf(breakShiftTravelCall
					.getNoOfBreaks());
		}
		...
		if (breakShiftTravelCall != null
				&& breakShiftTravelCall.getTotalBreakTime() != null) {
		...
		if (minutes < 1) {
			if (!numberOfBreaks.equalsIgnoreCase("None taken")) {
				numberOfBreaks = numberOfBreaks
						+ " totalling less than 1 minute";
			}
		}
		if (minutes > 1) {
			if (!numberOfBreaks.equalsIgnoreCase("None taken")) {
				numberOfBreaks = numberOfBreaks + " totalling " + minutes
						+ " minutes";
		...
		String dateText = "Shift started: "
				+ Utils.checkOutObject.getJobStartedTime() + "  Breaks: "
				+ numberOfBreaks;
		...
		if (Utils.checkOutObject.getJobSelected().equalsIgnoreCase(
			...
			mStartTravel.setText(getResources().getString(
					R.string.start_break_1));
		}
				...
				if (jsonObject.has(Constants.CAPTURE_VISTEC_SCREEN)) {
					if (jsonObject.getBoolean(Constants.CAPTURE_VISTEC_SCREEN)) {
						...
						return;
							...
							.getBoolean(Constants.RISK_ASSESMENT_SCREEN)) {
						Intent vistecIntent = new Intent(this,
	...
	private void sendLeaveWorkToServer() {
		...
		if (Utils.isInternetAvailable(this)) {
			...
			mSharedPref.saveWorkId(mContext, JobViewerDBHandler
					.getCheckOutRemember(mContext).getWorkId());

			Utils.SendHTTPRequest(
					mContext,
					CommsConstant.HOST
							+ CommsConstant.WORK_UPDATE_API
							+ "/"
							+ mSharedPref.getSharedPref(mContext).getString(
	...
	public void openWorkOptionsDialog() {
		...
		View view = ActivityPageActivity.this.getLayoutInflater().inflate(
