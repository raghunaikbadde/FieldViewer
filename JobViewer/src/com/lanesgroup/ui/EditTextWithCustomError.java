...
public class EditTextWithCustomError extends EditText {
	...
	private void init(Context context, AttributeSet attrs) {
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.EditTextWithCustomError);

		if (a.hasValue(R.styleable.EditTextWithCustomError_ErrorDefaultIcon)) {
			mErrorIcon = a
					.getDrawable(R.styleable.EditTextWithCustomError_ErrorDefaultIcon);
		}
		if (mErrorIcon == null) {
		}
		if (a.hasValue(R.styleable.EditTextWithCustomError_ErrorDefaultBackground)) {
			mErrorBackground = a
					.getDrawable(R.styleable.EditTextWithCustomError_ErrorDefaultBackground);
		}
		if (mErrorBackground == null) {
		}
		if (a.hasValue(R.styleable.EditTextWithCustomError_ErrorDefaultBackgroundAbove)) {
			mErrorBackgroundAbove = a
					.getDrawable(R.styleable.EditTextWithCustomError_ErrorDefaultBackgroundAbove);
		}
		if (mErrorBackgroundAbove == null) {
		}
		if (a.hasValue(R.styleable.EditTextWithCustomError_ErrorTextColor)) {
			mErrorTextColor = a.getColor(
					R.styleable.EditTextWithCustomError_ErrorTextColor,
					Color.rgb(50, 50, 50));
		}
	...
	private void setTheError(CharSequence error) throws NoSuchFieldException,
	...
	private void setTheErrorWasChanged(boolean value)
	...
	private void showError() {
		...
		mPopup.showAsDropDown(this, location[0], location[1]);
		// mPopup.showAsDropDown(this, -20, 0);
	...
	private int getErrorX() {
		...
		final Drawables dr = mDrawables;
		int xPos = getWidth() - mPopup.getWidth() - getPaddingRight()
				- (dr != null ? dr.mDrawableSizeRight : 0) / 2
				+ (int) (30 * scale + 0.5f);
		return -150;
	}
	...
	private int getErrorY() {
		 ...
		 * height is smaller.
		 */
		int vspace = getBottom() - getTop() - getCompoundPaddingBottom()
		 ...
		 * background.
		 */
		int yPos = icontop + (dr != null ? dr.mDrawableHeightRight : 0)
				- getHeight() - 2;
...
//			Log.v(Common.TAG, "top: " + location[1]);
			...
			mPopup.update(this, location[0], location[1], mPopup.getWidth(),
					mPopup.getHeight());
			// mPopup.update(this, -20, 0, mPopup.getWidth(),
			// mPopup.getHeight());
		}
